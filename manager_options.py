    ...
    if from_user.id in config.ADMINS:
        ...
        keyboard = [[InlineKeyboardButton(text="הוספת שורה", callback_data=strings.ADD_LINE),
                    ...
                    [InlineKeyboardButton(
                        text="ערוץ נוכחי: {}".format(user_data[strings.CHANNEL_LIST][user_data[strings.CHANNEL_INDEX]]),
                        callback_data=strings.CHANGE_CHANNEL)],
...
def manager_menu(bot, update, user_data):
    ...
    if strings.ADD_LINE in data:
        ...
        bot.send_message(chat_id=user_data[strings.CHANNEL_LIST][user_data[strings.CHANNEL_INDEX]],
                         text=user_data[strings.MESSAGE_TEXT],
                         reply_markup=InlineKeyboardMarkup(user_data[strings.KEYBOARD]))
        ...
        update.message.reply_text(
            "הערוץ שונה ל:{}".format(user_data[strings.CHANNEL_LIST][int(user_data[strings.CHANNEL_INDEX])]))
        load_settings(user_data)
...
def add_button_link(bot, update, user_data):
    ...
    row.append(button)
    user_data["row"] = row
    keyboard = [[InlineKeyboardButton(text="כן", callback_data=strings.ANOTHER_BUTTON_YES),
                 InlineKeyboardButton(text="לא", callback_data=strings.ANOTHER_BUTTON_NO)]]
...
def edit_keyboard_select(bot, update, user_data):
    ...
    if strings.EDIT_ROW in data:
        update.message.edit_text(text="בחר כפתור לערוך", reply_markup=InlineKeyboardMarkup(
            [user_data[strings.KEYBOARD][int(data.split('|')[1])]]))
        return "EDIT_BUTTON"
...
def load_settings(user_data):
    try:
        if user_data[strings.CHANNEL_INDEX] > len(user_data[strings.CHANNEL_LIST]):
            user_data[strings.CHANNEL_INDEX] = len(user_data[strings.CHANNEL_LIST])
        print(user_data)
        settings = user_data[user_data[strings.CHANNEL_LIST][int(user_data[strings.CHANNEL_INDEX])]]
        print("hh",settings)
        if len(settings) == 2:
...
def save_settings(user_data):
    user_data[user_data[strings.CHANNEL_LIST][user_data[strings.CHANNEL_INDEX]]] = [user_data[strings.KEYBOARD],
                                                                                    user_data[strings.MESSAGE_TEXT]]
